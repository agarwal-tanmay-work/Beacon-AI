...
import React, { useState, useRef, useEffect, useCallback, useMemo, createContext, useContext, memo } from "react";
        ...
        <div className="absolute top-full left-0 mt-1 bg-white rounded-lg shadow-lg overflow-hidden z-30 min-w-[120px]">
            <ul className="py-1">
                {menuOptions.map((option) => (
                    <li
                        key={option}
                        className={`px-4 py-2 hover:bg-gray-100 cursor-pointer text-black text-sm font-medium`}
...
const OptionTag = memo(({
    ...
    <div
        className={`flex items-center gap-1 bg-black/10 px-2 py-1 rounded-md text-xs text-black`}
        style={{ fontFamily: '"Inter", sans-serif' }}
    >
        <span>{option}</span>
        <button
            ...
            className={`h-4 w-4 flex items-center justify-center rounded-full hover:bg-black/20 text-black/70`}
        >
            <svg
                width="10"
                height="10"
    ...
    return (
        <div className="flex-1 relative h-full flex items-center">
            <textarea
                ...
                rows={1}
                className={`w-full min-h-8 max-h-24 bg-transparent text-sm font-normal text-left self-center text-black placeholder-[#6B7280] border-0 outline-none px-3 pr-10 py-1 z-20 relative resize-none overflow-y-auto`}
                style={{
...
const MenuButton = memo(({
    ...
    <div className="relative" ref={menuRef}>
        <button
            ...
            className={`h-8 w-8 flex items-center justify-center rounded-full bg-black/10 hover:bg-black/20 text-black transition-all ml-1 mr-1`}
        >
            <Plus size={16} />
        </button>
        <OptionsMenu
    ...
    return (
        <ChatInputContext.Provider value={contextValue}>
            <form
                ...
                className={`relative z-50 mx-auto min-h-12 w-full max-w-[600px] transition-all duration-${animationDuration} ease-out translate-y-0 opacity-100`}
                style={{
