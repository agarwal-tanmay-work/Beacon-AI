...
export default function OngoingReports() {
    ...
    const fetchReports = async () => {
        try {
            ...
            setReports(allReports.filter(r => r.status === 'Ongoing'));
            setError(null);
        } catch (err: unknown) {
            console.error(err);
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-white">Ongoing Reports</h1>
                    <p className="text-muted-foreground mt-1">Cases currently under investigation.</p>
                </div>
            ...
            <div className="rounded-xl border border-border bg-card overflow-hidden">
                <div className="p-6 border-b border-border">
                    <h3 className="font-semibold text-white">Ongoing Cases</h3>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="p-12 text-center text-muted-foreground">No ongoing reports found.</div>
                ) : (
                    <>
                        {/* Desktop Table */}
                            ...
                            {reports.map((report) => (
                                ...
                                >
                                    <div className="flex justify-between items-start">
                                        ...
                                        <span className={clsx("inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ring-1 ring-inset", getStatusColor(report.status))}>
                                            {report.status}
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between text-xs">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className={clsx("font-bold uppercase", getPriorityColor(report.priority))}>
                                                {report.priority}
                                            </span>
                                        </div>
                                    ...
                                    <div className="flex items-center justify-between pt-1">
                                        <div className="flex items-center gap-2 w-full">
                                            ...
                                            <span className="text-xs text-white font-medium">{report.credibility_score || 0}%</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
