...
interface CaseDetail {
    ...
    app_score_explanation: string | null;
                    ...
                    <div className="bg-card border border-border rounded-xl p-6">
                        ...
                        {caseData.ai_explanation && (
                            <div className="space-y-6 mb-6">
                                ...
                                {caseData.ai_explanation.limitations && caseData.ai_explanation.limitations.length > 0 && (
                                    <div className="p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/20">
                                        <div className="text-sm font-bold text-yellow-500 mb-1 flex items-center gap-2">
                                            <AlertTriangle className="w-4 h-4" /> Verifiability Constraints
                                        </div>
                                        <ul className="list-disc pl-5 space-y-1">
                                            {caseData.ai_explanation.limitations.map((l, i) => (
                                                <li key={i} className="text-xs text-yellow-200/80">{l}</li>
                                            ))}
                                        </ul>
                                    </div>
                                )}
                                ...
                                messages.map((msg) => (
                                    ...
                                    >
                                        ...
                                        <span className="text-[10px] text-muted-foreground mt-1">
                                            {msg.sender_role === 'admin' ? "You" : "USER"} • {formatToIST(msg.timestamp, { hour: undefined, minute: undefined, day: '2-digit', month: 'short', year: 'numeric' })}
                                        </span>
                                    </div>
                                ))
                            )}
