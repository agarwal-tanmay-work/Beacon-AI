...
class LLMAgent:
    ...
    async def chat(conversation_history: list, current_state: dict = None) -> Tuple[str, Optional[dict]]:
        ...
        # 3. API CALL - Single attempt (ai_service handles one quick internal retry)
        from app.services.ai_service import GeminiService
        try:
            # GeminiService.safe_chat handles one quick internal retry for 429/503
            text_response, retry_after = await GeminiService.safe_chat(messages, model=LLMAgent.GEMINI_MODEL, timeout=15.0)
            ...
            if text_response:
                ...
                print(f"[LLM_AGENT] API still overloaded after all retries. Retry-After: {retry_after}s", flush=True)
                return "I'm experiencing high demand right now. Please wait a moment and try again. Your message wasn't lost!", state
